  ...
  for (const { key, value } of options) {
    switch (key) {
      ...
      case "buf_validate":
        ...
        bufValidate = ["true", "1"].includes(value);
        break;
      default:
        throw new Error();
    }
  }
  return { jsonTypes, validTypes, bufValidate };
...
function generateMessageShape(
  ...
  for (const member of message.members) {
    ...
    if (member.kind === "field" && readOnlyFields.includes(member.localName)) {
      continue;
    }
...
function applyNestedConstraints(
  ...
  if (constraintsForField && constraintsForField.length > 0) {
